...
public class RE_ThirdPersonInput : ThirdPersonUserControl
{
    ...
    protected new void FixedUpdate()
    {
        ...
        bool aimPressed = Input.GetMouseButton(1);      // true en el frame en que se pulsa el botón de apuntar
        bool aiming = Input.GetMouseButton(1);                // true mientras el botón de apuntar esté pulsado
        bool dodging = Input.GetKey(KeyCode.G);
        ...
        if (Input.GetMouseButtonUp(1)) // Cuando apuntas para resetear y que no puedas caminar inclinado
        {
        ...
        if (dodging)
        {
            ...
            tpc.GetAnimator().SetBool("IsDodging", dodging);
            tpc.GetAnimator().SetBool("IsDodging",true);
        }

        else
        {
            tpc.GetAnimator().SetBool("IsDodging", false);
            tpc.GetAnimator().SetBool("IsWalking", walking);
        }
    }
